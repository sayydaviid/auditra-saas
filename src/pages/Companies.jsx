...
export default function Companies() {
  ...
  const filtered = useMemo(() => companies.filter((company) => {
    return `${company.name} ${company.cnpj} ${company.segment} ${company.responsible}`.toLowerCase().includes(search.toLowerCase());
  }), [search]);

  const activeCompanies = companies.filter((company) => company.status === 'Ativa').length;
  const totalProjects = companies.reduce((sum, company) => sum + company.projectsCount, 0);
  ...
  return (
    <div className="page-stack">
      ...
      <div className="stats-grid three-cards">
        <StatCard title="Empresas cadastradas" value={companies.length} subtitle="Clientes e instituições" icon={Building2} />
        <StatCard title="Empresas ativas" value={activeCompanies} subtitle="Com acesso liberado" icon={UsersRound} tone="green" />
        <StatCard title="Projetos vinculados" value={totalProjects} subtitle="Total na base" icon={Building2} tone="blue" />
      </div>

      <Card title="Consulta de empresas">
        <div className="filters-row single-filter">
          <Input label="Buscar" placeholder="Nome, CNPJ, segmento ou responsável" value={search} onChange={(event) => setSearch(event.target.value)} />
        </div>
      </Card>

      <Card title="Empresas clientes" description={`${filtered.length} empresa(s) encontrada(s)`}>
        <Table columns={columns} data={filtered} emptyTitle="Nenhuma empresa encontrada" />
      </Card>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={() => setModalOpen(false)}>Salvar visualmente</Button>
          </>
        }
      >
